...
import { readFileSync, existsSync, unlink, writeFileSync, appendFileSync } from 'fs';
...
class MyCustomReporter {
  constructor( globalConfig, options) {
    ...
    this.outputFiles = {
      ...
      xml: "./aman-test-cases.xml"
    };

    try {
      const data = readFileSync('../custom.ih', 'utf8')
      customData = data;
    } catch (err) {
      console.error(err)
    }
    ...
    for (let key in this.outputFiles) {
      if (existsSync(this.outputFiles[key])) {
        unlink(this.outputFiles[key], (err) => { if (err) console.log(`${this.outputFiles[key]} not deleted`); });
      }
    }
  ...
  onRunComplete(contexts, results) {
    ...
    if(createXMLFile){
      writeFileSync(this.outputFiles.xml, this.xml.toString({ pretty: true }));
    }
